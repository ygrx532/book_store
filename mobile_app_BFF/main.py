...
async def get_book_alt(isbn: str):
    """
    Proxy GET request to retrieve a book by its ISBN via the alternate Django endpoint.
    """
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise HTTPException(
                status_code=response.status_code if response is not None else 500,
                detail=f"Error from Book Service (alt): {exc}"
            ) from exc
        return response.json()
